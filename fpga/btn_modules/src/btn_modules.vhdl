...
begin
    ...
    D4: debouncer
        ...
        port map(
            clk => clk,
            btn => btns(3),
            btn_pressed => btns_pressed(3)
        );

    process(clk)
    begin
        btns_pressed(7 downto 4) <= (others => '0');
        if rising_edge(clk) then
            if (btns_pressed(0) = '1') then
                led_register(0) <= not led_register(0);
            end if;
            if (btns_pressed(1) = '1') then
                led_register(1) <= not led_register(1);
            end if;
            if (btns_pressed(2) = '1') then
                led_register(2) <= not led_register(2);
            end if;
            if (btns_pressed(3) = '1') then
