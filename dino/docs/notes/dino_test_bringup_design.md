...
implement those contracts). Status: DESIGN APPROVED PENDING RICO REVIEW.
...
### Harness API (harness.h)
    ...
    void bus_w_release(void);         // tri-state rig side, weak pullups on
...
`floats()` is the tri-state primitive: enable internal pullup, read; drive
brief pulldown via a 10k rig resistor pin, read. 1 then 0 = floating.
Every BIDIR contract line gets one.
...
### Pin budget (pinmap.h, single source of truth)
    ...
    Worst-case rig (MAR: 31 signals, control word: ~30) fits with >15 spare.
...
2. `tstate.walk`: pulse CLK, T0-3 counts 0..15 wrapping; TO decode one-hot
   (scope/LED taps — sampled by rig for completeness).
...
4. `taps`: CW9-15 pass-through: SA0-2, END, PC_UP, PC_MAR_MUX, HALT follow
   their CW bits 1:1 (verifies the strike-7 tap wires physically).
...
2. `word.split`: verify U9 = CW0-7 and U15 = CW8-15 (byte-role check —
   burn matters).
3. `image.crc`: walk all 4096 rows, CRC16 over the data, compare against
   the CRC the image generator printed. Full-ROM verify in-circuit.

### registers_a_b (A, B, C, OUT + '245s + LE stamps)
Rig drives: W0-7, /REG_x_LOAD, /REG_x_OUT, /REG_OUT_LOAD, CLK.
Samples: W (readback), OB0-7, MDR0-7 (the local bus), LE nets.
1. `stamp.gate`: LE_x = NOR(/LOAD, CLK): asserts only when /LOAD low AND
   CLK low. 4-row truth per register.
2. `load.readback` (per A/B/C): W=pattern, pulse load protocol (CLK low
   with /LOAD low, then CLK high), release W, assert /REG_x_OUT, read W.
3. `isolation`: load A=0xAA B=0x55, read both back; assert A unchanged.
4. `tristate`: no /OUT asserted -> W floats (floats() on all 8 bits).
...
6. `no.fight`: assert /REG_A_OUT then also load B in same state — legal
   ...
   the '245 CE = AND(load,out) logic isolates local bus correctly per the
   register rule).
...
3. `clear.merge`: PC_CLEAR high -> 0; RESET high -> 0 (both through the
   OR-from-NOR merge; verifies no bus fight on the merged net).
...
4. `write.window`: RAM write only during the WRITE_DIR + ~CLK window the
   sheet's gating defines — write attempt outside window leaves cell
   unchanged.
...
2. `INT-B registers+alu`: rig plays control_word (drives the strobe nets),
   executes LDAI 5; LDBI 3; SUB sequence by hand-strobing; assert A=2,
   FLAG_Z=0; then SUB to zero, FLAG_Z=1. Retires: shadow timing, two-edge
   commit discipline, W-bus sharing.
...
5. `INT-E fetch smoke test (near-full system)`: everything wired, rig owns
   only CLK + RESET, ROM burned with LDAI 0x42; OUT; HALT. Single-step ~12
   edges, at each edge sample W, M, IR, T against a cycle-accurate
   expectation table generated from the microcode. This is the dress
   rehearsal for free-run.
...
(CW0-7) + U15.bin (CW8-15) + printed CRC16s (consumed by `microcode.image.crc`)
+ diagnostic image mode (word = own address) for `addr.order`.
`asm/countdown.py` — hand-assembled demo -> program.bin, safe-fill = HALT
opcode. Burn all via TL866. (Implementation planned separately.)
